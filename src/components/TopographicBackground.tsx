...
export function TopographicBackground() {
  ...
  useEffect(() => {
    ...
    const noise = (x: number, y: number, t: number) => {
      ...
      for (let i = 0; i < 4; i++) {
        value += amplitude * Math.sin(x * scale * frequency + t * 0.5) * 
      ...
      for (let level = 0; level < levels; level++) {
        ...
        for (let x = 0; x < width / gridSize - 1; x++) {
          for (let y = 0; y < height / gridSize - 1; y++) {
            ...
            switch (code) {
              case 1: case 14: drawLine(px, left, bottom, py + size); break;
              case 2: case 13: drawLine(bottom, py + size, px + size, right); break;
              case 3: case 12: drawLine(px, left, px + size, right); break;
              case 4: case 11: drawLine(top, py, px + size, right); break;
              case 5:
                drawLine(px, left, top, py);
                drawLine(bottom, py + size, px + size, right);
                break;
              case 6: case 9: drawLine(top, py, bottom, py + size); break;

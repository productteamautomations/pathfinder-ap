    ...
    class WavyLine {
      ...
      update() {
        for (let i = 0; i < this.points.length; i++) {
          ...
          const wave =
            Math.sin(point.x * waveFrequency + timeRef.current + this.index * 0.3) *
            waveAmplitude;
          const dx = mouseRef.current.x - point.x;
          const dy = mouseRef.current.y - point.baseY;
          const normalizedDistance = Math.sqrt(
            Math.pow(dx / mouseRef.current.radiusX, 2) +
              Math.pow(dy / mouseRef.current.radiusY, 2)
          );
      ...
      draw(ctx: CanvasRenderingContext2D) {
        const thicknessVariation =
          Math.sin(timeRef.current * 0.5 + this.thicknessOffset) * 0.4;
        ...
        if (this.index === numLines - 1) {
          ...
          gradient.addColorStop(0, "rgba(227, 102, 79, 0.3)");
          gradient.addColorStop(1, "rgba(227, 102, 79, 0.1)");
